...
const homeDirectory = await invoke("get_home_directory");
const pathSeparator = await invoke("get_path_separator");
...
async function listDirectories() {
    ...
    const dirs = await invoke("list_directories", { path });
...
async function listMusicFiles() {
    const musicFiles = await invoke("get_music_files", { "path": path });
    const musicFilesDiv = document.querySelector("#musicfiles");
...
//=============================================================================
async function createMusicTable(musicFiles) {
    ...
    for (const [index, filePath] of musicFiles.entries()) {
        ...
        for (let rating = 1; rating <= 5; rating++) {
            ...
            // const fileSize = await invoke("get_file_size", { "pathname": filePath });
            // const lastModified = await invoke("get_modified_time", { "pathname": filePath });
            await invoke("get_rating", { "pathname": filePath }).then((currentRating) => {
        ...
        clearButton.addEventListener("click", () => {
            const radios = ratingTd.querySelectorAll("input[type='radio']");
        ...
        fileTd.textContent = filePath.split(pathSeparator).pop() || filePath;
        //
        // Assemble row
        //
...
//=============================================================================
async function playMusic(pathname) {
...
//=============================================================================
async function rateMusic(pathname, rating) {
    await invoke("rate_music_file", { "pathname": pathname, "rating": rating });
}
async function clearRating(pathname) {
